	...
	<script type="text/javascript">
        ...
        function getAllBusinessUnitInfo() {
            ...
            $('#table2').bootstrapTable({
                method: "post",
                url: webPath+"/svdm/unit/findUnitAll",
				dataType: "json",
...
//                        visible: false
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            var temp = "";
                            if (value == "稍有危险") {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;color:#66CC33'><b>一级</b></a>"
                            } else if (value == "一般危险") {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;color:#FFD700'><b>二级</b></a>"
                            } else if (value == "显著危险") {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;color:#FF8C00'><b>三级</b></a>"
                            } else if (value == "高度危险") {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;color:#FF0000'><b>四级</b></a>"
                            } else if (value == "极其危险") {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;color:#990000'><b>五级</b></a>"
                            }else if (value == "无危险") {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;'><b>无危险</b></a>"
                            }else {
                                temp = "<a class='fa fa-lock' href='javascript:void(0)' onclick='toSheet2Jsp(" + row.id + ","+row.rai_count + ")' style='font-size:13px;'><b>暂无评级</b></a>"
							}
                            return temp;
                        }
                    },
					{
                     title: '报告导出',
                        ...
                        formatter: function (value, row, index) {
                            var daochu = "";
                            daochu = "<button onclick='toDaoChu("+value+")'>导出</button>"
							return daochu;
                        }
					},
					{
        ...
        function getExport() {
			$.ajax({
                type: "GET",
                url: "http://localhost:5080/excel/create",
                async: false,
                success: function (data) {
					if(data == 'success'){
                        window.open("http://localhost:5080/excel/营运公司数据分析表.xls");
					}
                }
			})
        }
        function toSheet2Jsp(val,rai){
           window.location.href=webPath+"/formpages/sheet2.jsp?unit_id="+val+"&raiApple="+rai;
// 			window.open(webPath+"/formpages/sheet2.jsp?unit_id="+val);
        };
        function toDaoChu(val) {
            window.open(webPath+"/formpages/tu.jsp?unit_id="+val)
        };
        function goHistory() {
	...
	<div class="welcome" style="padding: 3em 0;">
		<div class="container" style="width:1400px;">
			...
			<div id="selectList" style="width: 100%;text-align: center;padding: 3%;}">
				...
				运输性质 ：
				<select id="yunshutype" style="width: 102px">
					<option value="请选择">请选择</option>
					<option value="single_unit_car.tonnage>0&&single_unit_car.seat=0">货运</option>
					<option value="single_unit_car.seat>0&&single_unit_car.tonnage=0">客运</option>
					<option value="single_unit_car.tonnage!=0&&single_unit_car.seat!=0">客运/货运</option>
				</select>
				...
				<%--<div style="float: right;">--%>
					<%--<button onclick="goHistory()">历史记录</button>--%>
				<%--</div>--%>
			</div>
			<div class="w3ls-row">
